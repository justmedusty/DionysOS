...
int32_t ahci_initialize(struct ahci_device *device){
    ...
    for(int i = 0; i < 32; i++){
        volatile struct ahci_command_header *header = P2V((((uint64_t) device->registers->command_list_base_address) | (uint64_t) device->registers->command_list_base_address_upper << 32) + ((i * sizeof(struct ahci_command_header)))) ;
    ...
    volatile struct ahci_command_header *header = P2V((((uint64_t) device->registers->command_list_base_address) | (uint64_t) device->registers->command_list_base_address_upper << 32) + ((command_slot * sizeof(struct ahci_command_header)))) ;
