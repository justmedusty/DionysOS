...
set_prdt(struct ahci_command_header *header, uint64_t buffer, uint32_t interrupt_vector, uint32_t byte_count) {
    volatile struct ahci_command_table command_table = {
            P2V(((uint64_t) header->command_table_base_address |
