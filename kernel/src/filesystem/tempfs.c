...
static uint64_t tempfs_read_bytes_from_inode(struct tempfs_filesystem* fs, struct tempfs_inode* inode, uint8_t* buffer, uint64_t buffer_size,
                                      uint64_t offset,uint64_t read_size_bytes);
static uint64_t tempfs_write_bytes_to_inode(struct tempfs_filesystem* fs, struct tempfs_inode* inode,uint8_t* buffer, uint64_t buffer_size,
                                      uint64_t offset,
...
void tempfs_mkfs(uint64_t ramdisk_id, struct tempfs_filesystem* fs) {
    ...
    struct tempfs_directory_entry **entries = kalloc(sizeof(struct tempfs_directory_entry) * TEMPFS_MAX_FILES_IN_DIRECTORY);

    tempfs_get_directory_entries(fs,entries,root.inode_number,TEMPFS_MAX_FILES_IN_DIRECTORY * sizeof(struct tempfs_directory_entry));
    ...
    tempfs_read_bytes_from_inode(fs,&root,buffer2,fs->superblock->block_size,0,len);
    serial_printf("STRING %s INO BLOCKS %i\n",buffer2,root.block_count);
    uint8_t *buffer3 = kalloc(PAGE_SIZE * 16);

    for(uint64_t i=0;i<10;i++) {
        tempfs_write_bytes_to_inode(fs,&root,buffer2,fs->superblock->block_size * 16,len + (len * i),len);
        tempfs_read_bytes_from_inode(fs,&root,buffer3,PAGE_SIZE * 16,0,root.size);
        serial_printf("STRING %s INO BLOCKS %i\n",buffer3,root.block_count);
        break;
    }
    tempfs_read_inode(fs,&root,root.inode_number);
    tempfs_read_bytes_from_inode(fs,&root,buffer3,PAGE_SIZE * 16,0,root.size);
...
static uint64_t tempfs_get_bytes_from_inode(struct tempfs_filesystem* fs, uint8_t* buffer, uint64_t buffer_size,
    ...
    struct tempfs_inode inode;
    tempfs_read_inode(fs, &inode, inode_number);
    return tempfs_read_bytes_from_inode(fs, &inode, buffer, buffer_size, byte_start % fs->superblock->block_size, size_to_read);
}
...
static uint64_t tempfs_get_directory_entries(struct tempfs_filesystem* fs,
    ...
    while (directory_entries_read <= (children_size / sizeof(struct tempfs_directory_entry)) && directory_entries_read <
        ...
        for (uint64_t i = 0; i < (fs->superblock->block_size / sizeof(struct tempfs_directory_entry)); i++) {
            ...
            *children[directory_entries_read++] = *(struct tempfs_directory_entry*)buffer[i * sizeof(struct
                tempfs_directory_entry)];
        }
    }
...
static uint64_t tempfs_read_bytes_from_inode(struct tempfs_filesystem* fs, struct tempfs_inode* inode, uint8_t* buffer, uint64_t buffer_size,
    ...
    if (!num_blocks_to_read && offset + read_size_bytes >= fs->superblock->block_size) {
    ...
    uint64_t end_offset = (read_size_bytes - start_offset) % fs->superblock->block_size;
    ...
    for (uint64_t i = start_block; i <= end_block; i++) {
        ...
        tempfs_read_block_by_number(current_block_number, buffer, fs, start_offset, fs->superblock->block_size - start_offset);

        bytes_read += (fs->superblock->block_size - start_offset);
...
static uint64_t tempfs_write_dirent(struct tempfs_filesystem* fs, struct tempfs_inode* inode,
    ...
    uint64_t entry_in_block = (inode->size % TEMPFS_MAX_FILES_IN_DIRECTORY) + 1;

    //allocate a new block when needed
    if(entry_in_block - 1 == 0) {
...
static uint64_t tempfs_write_bytes_to_inode(struct tempfs_filesystem* fs, struct tempfs_inode* inode,uint8_t* buffer, uint64_t buffer_size,
    ...
    if(end_block + 1 > inode->block_count){
        serial_printf("BLOCK NUMBERS %i %i %i %i %i\n",inode->blocks[0],inode->blocks[1],inode->blocks[2],inode->blocks[3],inode->blocks[4]);
    ...
    serial_printf("bytes_written %i size %i\n",bytes_written,inode->size);
    tempfs_write_inode(fs,inode);
    return SUCCESS;
}
...
static uint64_t tempfs_allocate_single_indirect_block(struct tempfs_filesystem* fs, struct tempfs_inode* inode,
    ...
    uint8_t* buffer = kalloc(PAGE_SIZE);
    uint64_t single_indirect = inode->single_indirect == TEMPFS_BLOCK_UNALLOCATED
                                   ? tempfs_get_free_block_and_mark_bitmap(fs)
