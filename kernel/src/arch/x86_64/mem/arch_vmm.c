...
void arch_init_vmm() {
    ...
    if (map_pages(kernel_pg_map->top_level, (k_text_start - kernel_min) + kernel_phys_min, (uint64 *) k_text_start, 0,(uint64) k_text_end - (uint64) k_text_start,1) == -1) {
        panic("Mapping text!");
    }
    serial_printf("Kernel Text Mapped\n");


    if (map_pages(kernel_pg_map->top_level, (k_rodata_start - kernel_min) + kernel_phys_min, (uint64 *) k_rodata_start,PTE_NX, (uint64) k_rodata_end - (uint64) k_rodata_start,1) == -1) {
        panic("Mapping rodata!");
    }
    serial_printf("Kernel Rodata Mapped\n");


    if (map_pages(kernel_pg_map->top_level, (k_data_start - kernel_min) + kernel_phys_min, (uint64 *) k_data_start,PTE_NX | PTE_RW, (uint64) k_data_end - (uint64) k_data_start,1) == -1) {
        panic("Mapping data!");
    }
    ...
    if (map_pages(kernel_pg_map->top_level, 0, 0 + hhdm_offset, PTE_RW | PTE_NX, 0x100000000,1) == -1) {
    ...
    for (uint64 i = 0; i < memmap->entry_count; i++) {
        ...
        for (uint64 j = base; j < top; j += PAGE_SIZE) {
            ...
            if (map_pages(kernel_pg_map->top_level, j, j + hhdm_offset, PTE_NX | PTE_RW, PAGE_SIZE,1) == -1) {
                panic("hhdm mapping");
            }
...
static pte_t *walkpgdir(p4d_t *pgdir, const void *va, int alloc,int init) {
    if (!pgdir) {
        return 0;
    }
...
int map_pages(p4d_t *pgdir, uint64 physaddr, uint64 *va, uint64 perms, uint64 size, int init) {
    ...
    for (;;) {
        if ((pte = walkpgdir(pgdir, (void *) address, 1,init)) == 0) {
            return -1;
        }
