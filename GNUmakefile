...
run-gdb: $(IMAGE_NAME).iso
	qemu-system-x86_64 -M q35 -m 2G -cdrom $(IMAGE_NAME).iso -boot d -serial mon:stdio -s -S
.PHONY: run-x86
run-x86: $(IMAGE_NAME).iso
	qemu-system-x86_64 -M q35,smm=off -smp 4 -m 4G -cdrom $(IMAGE_NAME).iso -boot d -monitor stdio
