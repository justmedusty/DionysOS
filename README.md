...
# NOTICE
In the current state, since the inital ramdisk size is static, you will need to allocate OVER 4196MB of memory to the machine. If there is less, when the ramdisk initialization routine is looking for consecutive max order buddy blocks it will just loop infinitely since there is not enough memory in the kernel pool to satisfy.
# Why Dionysos?
